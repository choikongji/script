echo ^<W-29^> >> ../result.xml
echo ^<Name^> Autologon 기능 제어 ^</Name^> >> ../result.xml
...
reg query "HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\Lsa" /v "restrictanonymous" >> ../result.xml
...
echo restrictanonymous = 0x0 사용 안함 >> ../result.xml
echo restrictanonymous = 0x1 사용 >> ../result.xml
echo ^</Comment^> >> ../result.xml

reg query "HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\Lsa" /v "restrictanonymoussam" | find "restrictanonymoussam" > test.txt
...
reg query "HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\Lsa" /v "restrictanonymous" | find "0x1" > nul
if !ERRORLEVEL! EQU 0 (
	if "!b!" EQU "0x1" (
		echo [양호] : 해당 보안 옵션 값이 설정 되어 있는 경우 >> ../result.xml
	)else (
